...
if __name__ == '__main__':
    ...
    descent.add_state('vy', fix_initial=False, fix_final=True, rate_source='a_y', ref0=0, ref=10)
    descent.add_state('energy', fix_initial=False, fix_final=False, rate_source='energy_dot', ref0=0, ref=1E7, defect_ref=1E4)
    ...
    descent.add_boundary_constraint('y', loc='final', lower=1,
                                  ref=1)  # Constraint for the final vertical displacement
    descent.add_boundary_constraint('x', loc='final', equals=1800,
                                  ref=100)  # Constraint for the final horizontal displacement
    descent.add_boundary_constraint('x_dot', loc='final', equals=1.,
                                  ref=1)  # Constraint for the final horizontal speed

    # Path Constraints
    descent.add_path_constraint('y', lower=0.1, upper=305,
                              ref=10)  # Constraint for the minimum vertical displacement
    descent.add_path_constraint('acc', upper=0.3,
                              ref=1.0)  # Constraint for the acceleration magnitude
    ...
    p.set_val('traj.descent.states:vy', descent.interpolate(ys=[10, 0], nodes='state_input'))
    p.set_val('traj.descent.states:energy', descent.interpolate(ys=[1E7, 1E3 ], nodes='state_input'))
